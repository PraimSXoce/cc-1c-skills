...
function Borrow-Form {
	...
	if ($srcChildItems) {
		...
		# Strip ChoiceParameterLinks blocks (contain text DataPaths that can't be resolved in extension context)
		$childItemsXml = [regex]::Replace($childItemsXml, '(?s)\s*<ChoiceParameterLinks>.*?</ChoiceParameterLinks>', '')
		$autoCmdXml = [regex]::Replace($autoCmdXml, '(?s)\s*<ChoiceParameterLinks>.*?</ChoiceParameterLinks>', '')
		# Strip FooterDataPath with Total* aggregate fields (not resolvable in extension)
		$childItemsXml = [regex]::Replace($childItemsXml, '\s*<FooterDataPath>[^<]*Total[^<]*</FooterDataPath>', '')
		$autoCmdXml = [regex]::Replace($autoCmdXml, '\s*<FooterDataPath>[^<]*Total[^<]*</FooterDataPath>', '')
