...
**⚠️ ВНИМАНИЕ: Используй ТОЛЬКО когда пользователь явно просит заимствовать реквизиты объекта.** В большинстве случаев НЕ нужен — новые элементы на формах создаются ПРОГРАММНО в модуле формы (через `Элементы.Добавить()`, `ИзменитьРеквизиты()`, `Команды.Добавить()` в перехватчике `&После("ПриСозданииНаСервере")`), а не декларативно в Form.xml.

**Два режима** (когда всё-таки нужен):
- `Form` (по умолчанию) — заимствует только те реквизиты объекта, которые уже выведены на форму
- `All` — заимствует все реквизиты и табличные части объекта

**Типовой сценарий** (добавление элементов на форму — БЕЗ -BorrowMainAttribute):
1. `/cfe-borrow` — заимствовать форму без реквизитов объекта
2. Написать код в Module.bsl — программно создать элементы в `&После("ПриСозданииНаСервере")`

**Редкий сценарий** (добавление нового реквизита в объект + вывод на форму):
...
3. Программно вывести реквизит на форму в Module.bsl
